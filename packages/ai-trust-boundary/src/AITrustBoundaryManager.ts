...
export class AITrustBoundaryManager {
   ...
   */
  public analyzePromptSafety(prompt: string): { isSafe: boolean; trustScore: number; reason?: string } {
   ...
   */
  public sanitizeDomContext(rawHtml: string): string {
   ...
   */
  public stripMaliciousInstructions(prompt: string): string {
    let clean = prompt
    for (const pattern of this.PROMPT_INJECTION_PATTERNS) {
   ...
   */
  public verifyOutputSafety(output: string): { isSafe: boolean; verifiedOutput: string } {
  ...
  public async routeWithFallback(
    prompt: string,
    primaryCall: (p: string) => Promise<string>,
    fallbackCall: (p: string) => Promise<string>
  ): Promise<string> {
